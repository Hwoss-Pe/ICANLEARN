...
@Component
public class Progress1ExpireKeyHandler implements RedisExpiredKeyHandler {
    private RedisUtil redisUtil;

    private  OccupationExplodeMapper occupationExplodeMapper;

    public Progress1ExpireKeyHandler(RedisUtil redisUtil, OccupationExplodeMapper occupationExplodeMapper) {
    ...
    public void handleExpiredKey(String expiredKey) {
        int id = Integer.parseInt(expiredKey.substring(RedisConstant.OCCUPATION_VALUES_1.length()));
        //获取备份key
        String backupKey = RedisConstant.OCCUPATION_VALUES_COPY_1 + id;
